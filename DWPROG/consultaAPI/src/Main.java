        ...
        for (String ip: ips) {
            try{
                ...
                for (int i = 1; i < 4; i++) {
                    ...
                    String userRespuesta = sc.nextLine();

                    if (userRespuesta.toLowerCase().equals("si")){
                        System.out.println("¿Quien es?");
                        String userResolucion = sc.nextLine();
                        boolean acierto = Boolean.parseBoolean(consumeAPI(ip, "resolver/" + userResolucion.toLowerCase()));
                        if (acierto){
                            puntuacion += puntuaciones[i-1];
                            consumeAPI("127.0.0.1", "score/jueves/"+ puntuacion);
                            numApis++;
                            break;
                        }
                    }
            ...
            System.out.println(numApis + " de " + ips.length + " adivinanzas");
        }
